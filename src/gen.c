...
static move_t *gen_piece_moves(const Position *pos, move_t *moves, bitboard_t filter, bool kingMoves)
{
    ...
    if (kingMoves) {
        ...
        moves = serialize_piece_moves(from, pos->pins, king, KingAttacks[from] & filter
            & ~pos->attacked, moves);
    }
